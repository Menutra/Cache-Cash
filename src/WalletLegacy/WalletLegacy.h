...
public:
  ...
  virtual TransactionId sendFusionTransaction(const std::list<TransactionOutputInformation>& fusionInputs, uint64_t fee, const std::string& extra = "", uint16_t mixIn = 0, uint64_t unlockTimestamp = 0);
  virtual TransactionId deposit(uint64_t term, uint64_t amount, uint64_t fee, uint16_t mixIn = 4) override;
